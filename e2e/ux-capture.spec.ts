...
// One output folder per run. Module-level so both viewport tests share it under
// --workers=1 (a single worker process imports this module once).
...
for (const vp of VIEWPORTS) {
  test(`ux capture @ ${vp.name}`, async ({ page }) => {
    ...
    await shot(page, entries, { state: 'start', viewport: vp.name, seed: RUN_SEED, how: 'boot /?seed=123 → .shell-start' });

    await page.locator('.shell-settings-open').click();
    await expect(page.locator('.shell-settings')).toBeVisible();
    await shot(page, entries, { state: 'settings', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-settings-open' });
    ...
    await shot(page, entries, { state: 'carpicker', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-cars (3D preview given 500ms to render)' });
    ...
    await shot(page, entries, { state: 'missions', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-missions-open' });
    ...
    await shot(page, entries, { state: 'store', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-store-open (fresh profile: starter car owned, rest priced)' });
    ...
    await shot(page, entries, { state: 'daily', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-daily-open (fresh profile: "not played yet" state)' });
    await page.locator('.shell-close-daily').click();

    // vs COMPUTER difficulty picker (EASY / MEDIUM / HARD).
    await page.locator('.shell-vscpu-open').click();
    await expect(page.locator('.bot-race-ui')).toBeVisible();
    await shot(page, entries, { state: 'vscpu-difficulty', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-vscpu-open → .bot-race-ui picker' });
    ...
    await shot(page, entries, { state: 'mp-race', viewport: vp.name, seed: RUN_SEED, how: 'start → .shell-mp-open → .mp-race-ui (HOST/JOIN + code input)' });
    ...
    await burst(page, entries, { state: 'racing', viewport: vp.name, seed: RUN_SEED, how: 'PLAY on /?seed=123, no input (auto-drives), burst from ~2s — 6 frames @ ~200ms, HUD visible' });
    ...
    await shot(page, entries, { state: 'pause', viewport: vp.name, seed: RUN_SEED, how: 'mid-run Escape → .shell-pause overlay' });
    ...
    await shot(page, entries, { state: 'gameover', viewport: vp.name, seed: CRASH_SEED, how: 'PLAY on /?seed=21 — straight run hits the seeded obstacle at ~3s (crash.spec.ts pattern)' });
    ...
    await shot(page, entries, { state: 'scores', viewport: vp.name, seed: CRASH_SEED, how: 'crash → MENU → .shell-leaderboard-open (populated by the seed-21 run)' });
    ...
    await shot(page, entries, { state: 'gameover-unlock', viewport: vp.name, seed: null, how: 'DEV-only /?screen=wipeout-unlock fixture (fullest WIPEOUT: rank + target + unlock + missions)' });
    ...
    await shot(page, entries, { state: 'mp-connection-test', viewport: vp.name, seed: null, how: '/?mp=1 → .mp-test overlay (URL-only entry)' });
    ...
    // steer) so the frames have a subject, then burst while STILL driving.
    ...
    try {
      ...
      await burst(page, entries, { state: 'zen', viewport: vp.name, seed: ZEN_SEED, how: `Zen on /?seed=${ZEN_SEED} (pinned world), OG-capture drive toward nearest landmark, burst while driving — HUD visible` });
    } finally {
      if (steering) await page.keyboard.up(steering);
